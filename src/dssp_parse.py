...
class DSSPData:
    ...
    def getAllResAccForChain(self, in_chain, delta):
        residue_acc = {}
        for i, chain_id in enumerate(self.chain):
            if chain_id is in_chain:
                residue_acc[str(int(self.getResnums()[i])+int(delta))] = self.getACC()[i]
        return residue_acc
